...
//사용자가 이기거나 비길 경우 대결을 반복 실행/질 경우 대결 종료
//대결 종료 후 사용자의 승리 횟수 출력
public class RpaGameApp {
	public static void main(String[] args) {
		Scanner scanner=new Scanner(System.in);
		
		//사용자 승리 횟수를 저장하기 위한 변수l
		int count=0;
		...
		while(true) {
			...
			//=> 1: 가위, 2: 바위, 3: 보
			//=> 1~3 범위의 정수값을 난수값으로 제공받아 변수에 저장
			int computer=(int)(Math.random()*3);
			
			//사용자로부터 키보드를 사용해 가위바위보 중 하나를 제공받아 변수에 저장
			//=> 키보드 입력값이 1~3 범위의 정수값이 아닌 경우 에러 메세지 출력하고 재입력
			...
			while(true) {
				...
				System.out.println("[Error]가위, 바위, 보 중 하나 중 선택하여 입력해 주세요.");
			}
			...
			//컴퓨터와 사용자의 가위바위보 대결의 승패를 비교하여 결과 출력
			if(computer == user) {
