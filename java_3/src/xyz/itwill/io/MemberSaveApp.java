...
public class MemberSaveApp {
	public static void main(String[] args) throws IOException {
		//파일에 원시데이터를 전달할 수 있는 출력스트림을 생성해 객체를 전달할 수 있는 출력스트림으로 확장
		ObjectOutputStream out=new ObjectOutputStream(new FileOutputStream("c:/data/member.txt"));
		
		//확장된 파일 출력스트림에 회원정보(Member 객체)를 전달하여 저장
		...
		//=> 출력스트림으로 전달되는 객체를 생성하는 클래스를 객체 직렬화 클래스로 작성하여 예외 미발생
		out.writeObject(new Member("abc123", "홍길동", "abc@itwill.xyz"));
		out.writeObject(new Member("qwe123", "임꺽정", "qwe@itwill.xyz"));
		out.writeObject(new Member("asd123", "전우치", "asd@itwill.xyz"));
