...
public class ObjectOutputStreamApp {
	public static void main(String[] args) throws IOException {
		//ObjectOutputStream 클래스 : 원하는 자료형의 값을 전달할 수 있는 출력스트림을 생성하기 위한 클래스
		//=> ObjectOutputStream(OutputStream out) 생성자로 ObjectOutputStream 객체 생성
		//=> 매개변수로 출력스트림(OutputStream 객체)을 전달받아 원하는 자료형의 값을 전달할 수 있는 
		//출력스트림으로 확장하기 위해 사용
		ObjectOutputStream out=new ObjectOutputStream(new FileOutputStream("c:/data/object.txt"));
		
		//ObjectOutputStream.writeObject(Object obj) : 매개변수로 전달받은 객체를 확장된 출력스트림으로
		//전달하기 위한 메소드
