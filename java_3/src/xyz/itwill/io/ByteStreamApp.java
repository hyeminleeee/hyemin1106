...
//java.io 패키지 : 입력 스트림과 출력 스트림을 생성할 수 있는 Java 자료형이 선언된 패키지

//스트림(Stream) : 값을 전달하기 위한 목적으로 선언된 입력클래스 또는 출력클래스로 생성된 객체
//=> 시냇물이 흐르는 모양을 모델링하여 만들어진 클래스로 한쪽방향으로 순차적으로 전달되도록 처리

//원시데이터 기반 스트림(Byte Stream) : 가공되지 않은 순수한 데이터(값)를 전달하기 위한
...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException {
		...
		//키보드 입력값(1Byte - 원시테이터)을 저장하기 위한 변수
		int readByte;
		
		while(true) {
			//System.in : 프로그램 개발을 위해 제공되는 키보드 입력 스트림이 저장된 필드
			//=> InputStream 클래스를 기반으로 생성된 입력스트림
			//=> 키보드를 누르면 키보드 입력값(1Byte)을 입력스트림으로 전달
			//InputStream.read() : 입력스트림에 저장된 원시데이터(1Byte)를 얻어와 정수값(int)
			//반환하는 메소드
			//=> 입력스트림에 입력값이 하나도 없는 경우 스레드가 일시 중지 상태로 전환
			...
			//입력종료 신호(Ctrl+Z : -1)가 전달된 경우 반복문 종료 
			if(readByte==-1) break;
			
			
			//System.out : 프로그램 개발을 위해 제공되는 모니터(콘솔) 출력스트림이 저장된 필드
			//=> PrintStream 클래스를 기반(OutputStream 클래스)으로 생성된 입력스트림
			//OutputStream.write(int b) : 매개변수로 전달받은 정수값을 출력스트림에 원시데이터
			//(1Byte)로 전달하는 메소드
