...
public class DataInputStreamApp {
	public static void main(String[] args) throws IOException {
		//DataInputStream 클래스 : 원하는 자료형의 값으로 전달받을 수 있는 입력스트림을 생성하기 위한 입력스트림
		//=> DataInputStream(InputStream in) 생성자를 사용하여 DataInputStream 객체 생성
		//=> 매려변수로 입력스트림(InputStream 객체)을 전달받을 원하는 자료형의 값을 전달받을
		//수 있는 입력스트림으로 확장
		DataInputStream in=new DataInputStream(new FileInputStream("c:/data/data.txt"));
		
		//출력스트림에 저장된 자료형의 순서대로 메소드를 호출하여 값을 반환받아야만 사용 가능
