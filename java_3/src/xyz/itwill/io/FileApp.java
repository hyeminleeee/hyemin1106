...
//=> pathname 매개변수에는 플랫폼의 파일 경로를 전달하여 사용

//파일경로 : 플랫폼의 파일(디렉토리)의 위치를 제공하기 위한
...
public class FileApp {
	public static void main(String[] args) throws IOException {
		...
		//하기 위해 [\] 기호 사용
		//=> Java 언어에서는 [\] 기호를 사용해 회피문자를 표현하므로 디렉토리와 파일을
		//구분하기 위해서는 [\\]를 형태로 작성해야 [\] 기호로 사용 가능 
		File fileOne=new File("c:\\data");
		
		//File.exists() : File 객체에 저장된 파일(디렉토리) 경로에 파일(디렉토리)이 없는 경우
		//[false]를 반환하고 파일(디렉토리)이 있는 경우 [true]를 반환하는 메소드
		if(fileOne.exists()) {
			System.out.println("c:\\data 폴더가 이미 존재합니다.");
		} else {
			//File.mkdir() : File 객체에 저장된 디렉토리 경로로 디렉토리를 생성하는 메소드
			fileOne.mkdir();
		...
		//Windows 운영체제를 제외한 나머지 운영체제에서는 디렉토리와 파을을 [/] 기호를 사용하여 구분
		//=> Java 언어에서는 Windows 운영체제를 사용해도 [\] 기호 대신 [/] 기호 사용해 디렉토리와 파일 구분 가능
		File fileTwo=new File("c:/data/itwill.txt");
		...
		if(fileTwo.exists()) {
			...
			System.out.println("c:\\data\\itwill.txt 파일을 생성했습니다..");
		}
		System.out.println("===================================================================");
		...
		//=> 파일이 없어 파일을 삭제하지 못한 경우 [false]를 반환하고 파일을 삭제한 경우 [true] 반환
		if(fileThree.delete()) {
			System.out.println("c:\\data\\itwill.txt 파일을 삭제 했습니다.");
		} else {
			System.out.println("c:\\data\\itwill.txt 파일이 없어 삭제 할 수 없습니다.");
		}
		System.out.println("===================================================================");
		...
		if(fileFour.exists()) {
			...
			//File.getAbsolutePath() : File 객체에 저장된 파일(디렉토리) 경로를 절대경로의 문자열로 
			//반환하는 메소드
		...
		//File.isDirectory() : File 객체에 저장된 파일(디렉토리) 경로가 디렉토리 경로인 경우 [true] 반환
		if (fileFive.isDirectory()) {
			//File.listFiles() : File 객체에 저장된 디렉토리 경로의 디렉토리에서 하위 파일(디렉토리)
			//경로가 저장된 File 객체가 요소에 저장된 배열을 반환하는 메소드
			File[] subFiles=fileFive.listFiles();
			
			for(File file : subFiles) {
				//File.isFile() : File 객체에 저장된 파일(디렉토리) 경로가 파일인 경우 [true]
				if(file.isFile()) {
