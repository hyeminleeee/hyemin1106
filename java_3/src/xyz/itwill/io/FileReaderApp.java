...
public class FileReaderApp {
	public static void main(String[] args) throws IOException {
		//FileReader 클래스 : 파일에 저장된 내용을 원시데이터로 제공받을 수 있는 입력스트림을
		//생성하기 위한 클래스
		...
		//모니터 출력스트림을 매개변수로 전달받아 문자데이터를 전달할 수 있는 출력스트림으로 확장
		OutputStreamWriter out=new OutputStreamWriter(System.out);
		
		System.out.println("[메세지]c:/char/byte.txt 파일에 저장된 내용입니다.");
		
		int readByte;
		
		while(true) {
			//파일 입력스트림을 사용해 파일에 저장된 내용을 읽어 문자데이터로 반환받아 변수에 
			//저장 - Load
			readByte=in.read();
			...
			//모니터(콘솔) 출력스트림으로 문자데이터를 전달하여 출력 처리
			//System.out.write(readByte); 원시데이터를 하나씩 출력 처리
			out.write(readByte);
			out.flush();//이거 없으면 Buffer에만 쌓이고 출력 안됨.
		}
