...
//스레드 동기화(Thread Synchronize) : 스레드가 메소드를 호출한 경우 메소드의 모든 명령을 실행하기
//전까지 메소드를 호출한 다른 스레드가 명령을 실행하지 못하도록 메소드 잠김(Lock) 기능 제공
//=> 메소드가 잠김 상태인 경우 메소드를 호출한 다른 스레드는 일시 중지 상태로 변환하고 메소드
