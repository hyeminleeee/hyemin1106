...
public class MessageSendApp {
	public static void main(String[] args) throws IOException {
		...
		//연결된 컴퓨터의 네트워크 식별자가 저장된 InetAddress 객체를 반환받아 저장
		InetAddress inetAddress=InetAddress.getByName("192.168.13.22");
		
		//전달할 문자열(메세지)를 byte 배열로 변환하여 전달
		...
		//int length, InetAddress address, int port) 생성자를 사용해 DatagramPacket 객체 생성
		DatagramPacket datagramPacket=new DatagramPacket(data, data.length, inetAddress, 4100);
		
		//DatagramPacket.send(DatagramPacket packet) : 매개변수에 전달된 DatagramPacket 객체의
