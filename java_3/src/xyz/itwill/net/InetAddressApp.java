...
//네트워크(Network) : 두 대 이상의 컴퓨터에서 값을 송수신할 수 있는 기능을 제공하는 환경
...
//=> 인터넷을 사용하기 위한 네트워크 주소(IP Address)에 대한 통신 규약 - IPV4, IPV6
...
//=> 사설 IP 주소 : A Clss(10.0.0.0 ~ 10.255.255.255), B Class(172.16.0.0 ~ 172.31.255.255)
...
//Gateway 주소 : 라우터 통신장비에 부여된 IP 주소 - 이더넷 장치에 설정하기 위한 주소

//도메인(Domain) : 인터넷을 사용하기 위해 그룹(개인)에게 부여하는 문자로 구성된 네트워크 주소
//=> IP 주소 대신 도메인을 사용한 호스트명을 이용해 네트워크 사용

//DNS Server : 도메인(호스트)을 IP 주소로 변환하는 서비스를 제공하는 컴퓨터

//서버(Server) : 네트워크 서비스를 제공하기 위한 컴퓨터
//클라이언트(Client) : 네트워크 서비스를 제공받기 위한 컴퓨터

//전송 계층 - TCP or UDP
//TCP(Transmission Control Protocol) : 연결형 프로토콜로 신뢰할 수 있는 데이터 전송을 위한 프로토콜
//=> 연결형 프로토콜 : 호스트와 호스트가 1:1 관계로 연결된 후 데이터 전송 기능 제공 - 소켓(Socket) 통신
//UDP(User Datagram Protocol) : 비연결형 프로토콜로 신뢰할 수 없는 데이터 전송을 위한 프로토콜
//=> 비연결형 프로토콜 : 호스트가 1:n 관계로 연결된 후 데이터 전송 기능 제공 - 고속 통신

//응용 계층 - FTP, HTTP, SMTP, POP3, IMAP 등
//=> 전송 계층의 프로토콜을 사용해 다양한 네트워크 서비스를 제공하기 위한 프로토콜 

//포트번호(Port Number) : 데이터를 송수신하기 위한 고유의 통신경로
...
//패킷(Packet) : 인터넷상에 데이터를 송수신하기 위한 단위
//=> Java 언어에서는 패킷(Packet)을 byte 배열로 생성하여 사용
...
public class InetAddressApp {
	public static void main(String[] args) throws UnknownHostException {
		...
		//현재 사용중인 컴퓨터는 기본적으로 [127.0.0.1]의 IP 주소(LoopBack)라 제공되며
		//[localhost]라는 호스트명 사용
		...
		//InetAddress.getHostAddress() : InetAddress 객체에 저장된 IP주소를 문자열로 반환하는 메소드
		System.out.println("myComputer.getHostAddress() = "+myComputer.getHostAddress());
		System.out.println("=========================================================");
		//InetAddress.getByName(String name) : 매개변수로 전달받은 호스트이름(IP 주수)에 대한
		...
		for(InetAddress address : naver) {
			System.out.println("[www.itwill.xyz] 컴퓨터의 IP 주소 = "+address.getHostAddress());
		}
