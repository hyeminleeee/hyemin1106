...
//Panel 클래스 : 컴포넌트를 배치하여 그룹화하기 위한 클래스 - 종속
...
public class PanelApp extends Frame {
	...
	public PanelApp(String title) {
		...
		//TextArea 클래스 : 여러 줄의 문자열을 입력 또는 출력 처리하기 위한 컴퍼넌트를 구현한 클래스
		TextArea textArea=new TextArea();
		
		//TextField 클래스 : 한줄의 문자열을 입력 또는 출력 처리하기 위한 컴퍼넌트를 구현한 클래스
		...
		//=> Font 클래스의 Font(String name, int style, int size) 생성자를 사용하여 Font 객체는 생성
		...
		textField.setFont(new Font(Font.SANS_SERIF, Font.PLAIN+Font.ITALIC, 25));
		...
		//Component.setBackground(Color c) : 컴포넌트의 배경색을 변경하는 메소드
		//=> Color 객체는 Color 클래스의 상수필드로 사용 가능
		textArea.setBackground(Color.YELLOW);
		
		//Component.setEnalbe(boolean b) : 컴퍼넌트의 활성화 및 비활성화를 변경하는 메소드
