...
public class DecimalFormatApp {
	public static void main(String[] args) {
		...
		//=> 패턴문자 : #(숫자 - 공백), 0(숫자 - 0), 콤마(,), 소수점(.), 달러($)
		DecimalFormat decimalFormat=new DecimalFormat("###,###,##0");
		
		//DecimalFormat.format(Number numver) : 매개변수로 전달받은 Number 객체에 저장된 숫자값을
		...
		//=> 3자리마다 , 기호를 사용하여 숫자값을 표현한 패턴정보가 저장된 DecimalFormat객체 반환
		System.out.println("금액 = "+DecimalFormat.getInstance().format(money)+"원");
		
		//DecimalFormat.getInstance() : 패턴정보가 저장된 DecimalFormat 객체를 반환하는 메소드
		//=> 맨 앞자리에 플랫폼에서 사용하는 화패단위를 부치고 3자리마다 , 기호를 사용하여
		//숫자값을 표현한 패턴정보가 저장된 DecimalFormat객체 반환
