...
public class ListApp {
	public static void main(String[] args) {
		...
		//요소값(객체)을 반환하는 메소드
		//=> 매개변수로 전달받은 첨자가 List 객체의 요소값을 구분하는 첨자의 범위를 벗어난 경우
		//IndexOutOfBoundsException 발생
		System.out.println("3번째 사용자의 이름 = "+list.get(2));
		//System.out.println("10번째 사용자의 이름 = "+list.get(9));
		System.out.println("======================================================================");
		//List.add(int index, E element) : List 객체에 매개변수로 전달받은 첨자 위치에 요소값(객체)을
		//삽입하여 저장하기 위한 메소드
		list.add(3, "일지매");
		System.out.println("list = "+list);
		System.out.println("======================================================================");
		...
		//List.remove(int index) : List 객체에서 매개변수로 전달받은 첨자 위치의 요소값을 삭제
		list.remove(4);
		System.out.println("list = "+list);
		System.out.println("======================================================================");
		...
		//List.iterator() : List 객체에 저장된 모든 요소값을 일괄 처리할 수 있는 Iterator 객체
		...
		System.out.println("list = "+list);
		System.out.println("======================================================================");
		//List.isEmpty() : List 객체에 저장된 요소값이 있는 경우 [false]를 반환하고 요소값이 하나도
		//없는 경우 [true]를 반환하는 메소드
		if(list.isEmpty()) {//if(list.size() == 0) 명령과 동일
			System.out.println("List 객체에 저장된 요소값이 하나도 없습니다.");
		}
		System.out.println("======================================================================");
		
		//Arrays.asList(E ... elements) : 매개변수로 0개 이상의 객체를 전달받아 List 객체에 
		//요소값으로 저장하여 List 객체를 반환하는 정적메소드
