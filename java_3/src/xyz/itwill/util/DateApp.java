...
//Date 클래스 : 날짜와 시간을 저장한 객체를 생성하기 위한 클래스
...
public class DateApp {
	...
	public static void main(String[] args) {
		...
		//Date 객체가 저장된 참조변수를 출력하면 자동으로 toString() 메소드 호출(생략가능)
		...
		//Date.getMonth() : Date 객체에 저장된 날짜와 시간 중 [월]를 반환하는 메소드
		//=> 0(1월) ~ 11(12월) 범위의 정수값 반환
		//Date.getDate() : Date 객체에 저장된 날짜와 시간 중 [일]를 반환하는 메소드
		//Date.getDay() : Date 객체에 저장된 날짜와 시간 중 [요일]를 반환하는 메소드
		//=> 0(일) ~ 6(토) 범위의 정수값 반환
		...
		//Date.getTime() : Date 객체에 저장된 날짜와 시간을 시간값(TimeStamp)로 변환하여 반한하는 메소드
		long currentTime=now.getTime();//long currentTime=System.currentTimeMillis();
		
		//new 연산자로 Date 클래스의 Date(int year, int month, int date) 생성자를 호출하여 객체를 생성
		//=> [2000년 1월 1일 0시 0분 0초]의 날짜와 시간이 저장된 Date 객체 생성
		Date wantDate=new Date(96, 10, 6);
