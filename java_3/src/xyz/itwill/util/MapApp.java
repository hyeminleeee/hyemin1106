...
public class MapApp {
	public static void main(String[] args) {
		...
		//=> Map 객체에 저장된 엔트리의 이름(Key)은 내부적으로 Set 객체로 저장되어 있으므로 동일한
		//이름(Key)의 엔트리는 중복되지 않으며 순서 없이 저장
		map.put(1000, "홍길동");
		map.put(2000, "임꺽정");
		map.put(3000, "전우치");
		map.put(4000, "일지매");
		...
		System.out.println("Map = "+map);
		System.out.println("===================================================================");
		//Map.get(K Key) : Map 객체에 저장된 엔트리 중 매개변수로 전달받은 이름의 엔트리 값(Value)을
		//반환하는 메소드
		...
		System.out.println("===================================================================");
		//Map.valuse() : Map 객체에 저장된 모든 엔트리의 값(Value)가 저장된 Collection 객체
		//(List 객체)를 반환하는 메소드
