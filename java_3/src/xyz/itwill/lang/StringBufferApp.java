...
//StringBuilder 클래스 : 문자열을 저장한 객체를 생성하기 위한 클래스
//=> StringBuffer 클래스와 동일한 메소드를 제공하지만 StringBuilder 클래스의 메소드는 일반
//메소드로 선언되어 있지만 StringBuilder 클래스는 동기화 메소드로 선언
...
public class StringBufferApp {
	public static void main(String[] args) {
		...
		//매개변수로 전달받은 객체(값)을 삽입하는 메소드
		sb.insert(4, "X");
		System.out.println("sb = "+sb);//sb=ABCDXEF
		System.out.println("======================================================================");
		
		//StringBuffer.deleteCharAt(int index) : StringBuffer 객체에 저장된 문자열에 매개변수로 
		//전달받은 객체(값)을 삭제하는 메소드
		sb.deleteCharAt(2);
		System.out.println("sb = "+sb);//sb=ABDXEF
		System.out.println("======================================================================");
		
		//StringBuffer.delete(int BeginIndex, int endIndex) : StringBuffer 객체에 저장된 문자열에
		//매개변수로 전달받은 시작첨자 위치의 문자부터 종료첨자 위치의 문자전까지의 문자들을
		//메소드 - 원하는 범위의 문자열 삭제
		//삭제하는 메소드
		sb.delete(4, 6);
		...
		//StringBuffer.reverse() : StringBuffer 객체에 저장된 문자열의 문자를 반대로 나열하여 저장
		sb.reverse();
