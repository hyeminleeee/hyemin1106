...
public class DigitalClockApp extends JFrame{
	...
	public DigitalClockApp(String title) {
		...
		*/
		...
		clockLabel.setFont(new Font("궁서체", Font.BOLD, 30));
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while(true) {
					//isRun 필드값이 [true]인 경우에만 플랫폼의 현재 날짜와 시간을 제공받아
					//JLabel 컴퍼넌트에 문자열 변경
					if(isRun) {
						Date date=new Date();
						String printDate=dateFormat.format(date);
	...
	*/
	...
	//=> JButton 컴퍼넌트를구분하여 isRun 필드값 변경 처리 
	public class ClockButtonEventHandle implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			Object evetSource=e.getSource();
			
			if(evetSource == startButton) {
				startButton.setEnabled(false);
				stopButton.setEnabled(true);
				
				isRun=true;
			} else if(evetSource == stopButton) {
