...
//JTextField 컴퍼넌트에 문자열(메세지)을 입력한 후 엔터를 누르면 JTextArea 컴퍼넌트에 메세지를
//추가하여 출력하는 이벤트 처리 프로그램 작성
...
public class SwingApp extends JFrame {
	...
	public SwingApp(String title) {
		...
		//Swing 컴퍼넌트는 플랫폼에서 제공하는 모든 글꼴 사용 가능
		textArea.setFont(new Font("바탕체", Font.BOLD, 20));
		textField.setFont(new Font("바탕체", Font.BOLD, 20));
		...
		//=> JFrame 객체를 사용하여 컴퍼넌트 배치, 배경색 변경, 배치관리자 변경 등을 설정하지 않고
		//JFrame 객체의 Container 객체에 컴퍼넌트 배치, 배경색 변경, 배치관리자 등 설정
		Container container=getContentPane();
		
		//JTextArea 컴퍼넌트는 폭 또는 높이를 벗어날 경우 스크롤이 미생성
		//=> 문자열이 JTextArea 컴퍼넌트의 폭 또는 높이를 벗어날 경우 스크롤이 생성되도록 JScrollPane
		//클래스로 객체(컨테이너)를 생성하여 JTextArea 컴퍼넌트를 배치 처리
