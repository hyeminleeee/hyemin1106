...
//위해 예외처리(ExceptionHandle) 필요

//Java 언어에서는 예외 관련 정보를 제공하기 위해 다양한 예외클래스를 제공
//예외클래스(Exception Class) : 예외처리에 관련된 정보를 제공하기 위한 클래스
//=> Exception 클래스를 상속받아 작성된 자식클래스
//=> 프로그램 실행시 예외가 발생되면 JVM은 예외클래스로 예외정보가 저장된 예외 객체 생성

//Throwable 클래스 : 프로그램에서 발생되는 모든 오류정보를 저장하기 위한 부모클래스
//=> 자식클래스 : Error 클래스, Exception 클래스

//예외의 종류
//1. 일반 예외 : 프로그램 실행시 예외가 발생될 가능성이 높아 예외처리하지 않으면 컴파일시 에러 발생
//=> RuntimeException 클래스를 상속받지 않는 예외클래스로 객체 생성
//2. 실행 예외 : 프로그램 실행시 예외가 발생될 가능성이 낮아 예외처리하지 않아도 컴파일시 에러 미발생
//=> RuntimeException 클래스를 상속받은 예외클래스로 객체 생성
//=> 실행 예외를 예외처리 하지 않으면 JVM이 예외클래스로 객체를 생성하여 예외객체에 저장된 정보로 
...
//=> try 블럭의 명령 실행시 예외가 발생되면 catch 블럭으로 스레드를 가져와 예외처리 명령을
//실행하므로 try 블럭에서 예외가 발생된 명령 하단에 작성된 명령 미실행
//=> catch 소괄호의 작성된 예외클래스의 참조변수에는 예외클래스의 객체를 전달받아 저장
//=> catch 블럭은 여러번 작성 가능
