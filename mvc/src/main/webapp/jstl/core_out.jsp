...
<body>
	...
	<p>정수값 = ${num }</p>
	<%-- out 태그 : 값(객체)을 출력 처리하기 위한 태그 --%>
	<%-- value 속성 : 출력될 값(객체)을 속성값으로 설정 --%>
	<%-- => value 속성값으로 EL을 사용해 Scope 속성값으로 값(객체)를 제공받아 설정 가능 --%>
	<p>정수값 = <c:out value="${num }"/></p>
	<hr>
	<%-- set 태그를 사용해 Scope 속성값으로 HTML 태그가 포함된 문자열을 저장 --%>
	<c:set var="html" value="<font size='7' color='red'>안녕하세요.</font>"/>
	<%-- EL만 사용하면 Scope 속성값이 HTML 태그가 포함될 문자열인 경우 HTML 태그로 동작되어 
