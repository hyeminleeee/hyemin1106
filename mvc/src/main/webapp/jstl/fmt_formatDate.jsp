...
<body>
	...
	<hr>
	<%-- 서버 플랫폼의 현재 날짜와 시간이 저장된 Date 객체를 Scope 속성값으로 저장 --%>
	<c:set var="now" value="<%=new Date() %>"/>
	<%-- El을 사용해 Scope 속성값(Date 객체)을 출력 처리하면 Date 클래스의 toString() 메소드를
	...
	<%-- value 속성 : Date 객체를 속성값으로 설정 - EL을 사용해 Date 객체를 제공받아 속성값으로 설정 가능 --%>
	...
	<%-- => dateStyle 속성값을 [full]로 설정한 경우 [yyyy년 M월 d월 E요일] 패턴의 문자열로 변환되어 제공 --%>
	...
	<p>now(시간) = <fmt:formatDate value="${now }" type="both" timeStyle="short"/></p>
	<p>now(시간) = <fmt:formatDate value="${now }" type="both" timeStyle="full"/></p>
