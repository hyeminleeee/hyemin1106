...
<%-- Functions 태그 라이브러리의 커스텀 태그를 JSP 문서에서 사용할 수 있도록 설정 - 접두사로 [fn] 사용 --%>
...
<body>
	...
	<hr>
	<%-- substring 함수 : 매개벼수로 전달받은 문자열을 시작첨자(첨자 위치의 문자 포함)와 
	...
	<div>${content }</div>
	<%-- replace 함수 : 매개변수로 전달받은 문자열에서 검색 문자열을 찾아 치환 문자열로 변경하여
	반환하는 함수 --%>
	...
	<%-- out 태그를 사용하면 HTML 태그가 포함된 문자열을 그대로 출력 처리 - XXS 공격에 대한 방어 --%>
