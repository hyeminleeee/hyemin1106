...
<body>
	...
	<p>자동차(색상) = ${car.getCarColor() }</p>
	<hr>
	<%-- ${속성명.필드명} 형식으로 EL 표현식을 사용해 속성값으로 객체를 제공받아 . 연산자를
	사용해 객체의 필드명으로 Getter 메소드를 호출하여 필드값을 반환받아 출력 처리 가능 --%>
	<%-- => 클래스의 Getter 메소드를 호출하지 않아도 객체의 필드값을 반환받아 출력 처리 --%>
	<%-- => Getter 메소드를 규칙에 맞지 않게 작성하거나 Getter 메소드가 없는 경우 에러 발생 --%>
	<p>자동차(모델명) = ${car.modelName }</p>
	<p>자동차(색상) = ${car.carColor }</p>
	<hr>
	<%-- . 연산자 대신 [] 연산자를 사용해 객체의 필드값을 제공받아 출력 처리 가능 --%>
	<%-- => [] 연산자 내부에 "" 기호로 필드명을 사용해 Getter 메소드를 호출해 필드값을 반환받아 출력 처리 --%>
	<p>자동차(모델명) = ${car["modelName"] }</p>
	<p>자동차(색상) = ${car["carColor"] }</p>
	<hr>
