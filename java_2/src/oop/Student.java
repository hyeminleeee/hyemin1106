...
//static : 객체가 아닌 클래스로 필드 또는 메소드에 접근하여 사용하기 위한 기능 제공 키워드(제한자)
//=> 클래스(내부클래스-중첩클래스), 필드, 메소드 작성시 사용하는 제한자
...
public class Student {
	//인스턴스 필드(Instance field) : 객체 생성시 객체에 만들어지는 필드로 객체를 사용하여 필드 사용
	//=> 생성자를 사용하여 객체 생성시 필드에 초기값 저장
	private int num;
	private String name;
	private int kor, eng, tot;
	
	//정적 필드(Static field) : 클래스 파일(XXX.class)을 읽어 메모리(MethodArea)에 저장될 때
	//만들어지는 필드로 클래스로 사용하여 필드에 접근
	...
	//인스턴스 메소드 : this 키워드를 제공받아 사용할 수 있는 메소드로 객체를 사용하여 호출
	public int getNum() {
		return num;
	}
	...
	//정적 메소드 : this 키워드를 제공받지 않는 메소드로 클래스를 사용하여 호출
	//=> 인스턴스 필드 및 인스턴스 메소드 사용 불가능
