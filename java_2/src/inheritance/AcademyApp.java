...
public class AcademyApp {
	public static void main(String[] args) {
		...
		*/
		...
		//=> 배열의 요소(부모클래스)에는
		//학생정보(AcademyStudent 객체 ), 강사정보(AcademyInstructor 객체),
		...
		//배열 요소에 저장된 부모클래스의 객체를 하나씩 제공받아 참조변수에
		//저장하여 부모클래스의 객체를 참조하며 부모클래스의 메소드를
		//일괄적으로 호출하기 위한 반복문
		for(AcademyPerson person : persons) {
			//오버라이딩 선언되지 않은 자식클래스의 메소드를 호출하기 위해서는 명시적 객체
			//형변환을 사용하여 부모클래스의 참조변수로 자식클래스의 객체를 참조해 자식클래스의
			//메소드 호출
			...
			//=> 참조변수로 객체 형변환 가능한 클래스가 아닌 경우 [false]를 제공하고
			//객체 형변환 가능한 클래스인 경우 [true]를 제공하는 연산자
			if(person instanceof AcademyStudent) {
				System.out.println(((AcademyStudent)person).getCourse()+" 학생정보 >> ");
			} else if(person instanceof AcademyInstructor) {
				System.out.println(((AcademyInstructor)person).getSubject()+" 강사정보 >> ");
			} else if(person instanceof AcademyStaff) {
				System.out.println(((AcademyStaff)person).getDepart()+" 직원정보 >> ");
			}
			
			//오버라이딩 선언된 자식클래스의 메소드는 묵시적 객체 형변환에 의해 부모클래스의
			//참조변수가 일시적으로 자식클래스의 객체를 참조하여 자식클래스의 메소드를 호출
			//=>부모클래스의 참조변수가 상속관계의 자식클래스의 객체로 자동 형변환되어
