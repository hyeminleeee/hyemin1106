...
//=> 반드시 함수형 인터페이스로만 람다 표현식 사용하여 객체 생성 가능

//람다표현식을 사용하여 익명클래스로 객체를 생성하는 방법
//형식) (자료형 변수명, 자료형 변수명, ...) -> { 명령; 명령; ... };
//=> 함수형 인터페이스에 작성된 추상메소드의 반환형 및 메소드명 생략
//=> 소괄호 기호( )안에 함수형 인터페이스에 작성된 추상메소드의 매개변수 작성
//=> 매개변수의 자료형을 생략할 수 있으며 매개변수가 하나인 경우 소괄호 기호 생략 가능
//=> 블럭 기호{ }에 함수형 인터페이스의 추상메소드를 오버라이딩 선언하여 실행될 명령 작성
//=> 블럭 기호에 작성된 명령이 하나인 경우 블럭 생략 가능하지만 return 키워드에 의해 
