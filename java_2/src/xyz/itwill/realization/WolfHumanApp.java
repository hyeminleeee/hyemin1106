...
//				public static final 자료형 필드명(상수필드)=초기값; //public static final 생략 가능
//				...
//				public abstract 반환형 메소드명(자료형 변수명, 자료형 변수명, ...); public abstract 생략가능
//				...
//		}
...
//=> 클래스는 implements 키워드를 사용하여 다수의 인터페이스 상속 가능 - 다중 상속
//=> 인터페이스를 상속받은 클래스는 상속받은 인터페이스의 모든 추상메소드를 무조건 오버라이딩 선언
//=> 인터페이스로 참조변수를 생성하여 자식클래스의 생성자로 객체를 생성하여 저장 가능 - 묵시적
...
//인터페이스를 선언하여 클래스 작성시 상속 받아야 되는 이유?
//1. 클래스의 단일 상속 관련 문제를 일부 보완하기 위해
//ex) public class 늑대인간 extends 인간, 늑대; => 불가능 : 에러(인간과 늑대를 동시에 상속받을 수 없음)
//	  public class 늑대인간 extends 인간 implements 늑대 => 가능
//	  public class 늑대인간 extends 인간 implements 늑대, 흡혈귀 => 가능
//2. 클래스 선언시 반드시 작성해야될 메소드를 제공하기 위해 인터페이스 사용 - 작업지시어
//ex) TV or Radio and SmartPhone >> 볼륨 증가, 볼륨 감소
//=> 인터페이스를 상속받은 모든 자식클래스에 동일한 메소드가 선언되도록 규칙 제공
//=> 클래스 간의 결합도를 낮춰 클래스 교체에 따른 기존 클래스의 영향 최소화 - 유지보수의 효율성
...
public class WolfHumanApp {
	public static void main(String[] args) {
		...
		//할 수 있으며 상속관계에 의해 부모클래스의 객체를 참조해 부모클래스의 메소드 호출 가능
		...
		//자식클래스의 생성자를 호출하여 부모클래스의 객체와 자식클래스의 객체를 모두 생성하여
		//부모클래스의 참조변수에 저장
		//=> 부모클래스의 참조변수로 부모클래스의 객체를 참조하여 부모클래스의 메소드를 호출 가능
