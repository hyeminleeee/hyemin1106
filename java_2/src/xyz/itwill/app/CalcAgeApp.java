...
public class CalcAgeApp {
	public static void main(String[] args) {
		...
		//=> Date 클래스의 기본 생성자를 이용하여 객체를 생성할 경우 프로그램을 실행한 플렛폼
		//(OS - 운영체제)의 현재 날짜와 시간이 저장된 Date 객체 생성 
		Date now=new Date();
		
		//Date.getYear() : Date 객체에 저장된 날짜와 시간에서 년도를 얻어와 반환하는 메소드
		//=> Date 객체에 저장된 년도는 1900년을 기준으로 1년에 1씩 증가된 정수값 저장되어 있으므로
		//반드시 1900을 더해야 원하는 년도를 제공받아 사용 가능
		...
		System.out.println("[결과]"+name+"님의 나이는 "+(2024-year)+"살 입니다."); 내년엔 못씀;
		scanner.close();*/
	}
}
