...
//GUEST 테이블에 행(게시글)을 삽입, 변경, 삭제, 검색하는 기능을 제공하는 클래스
public class GuestDAO extends JdbcDAO {
	...
	//게시글의 글번호(int)을 전달받아 GUEST 테이블에 저장된 행을 삭제하고 삭제행의 갯수(int)를 반환하는 메소드
	...
	//게시글의 글번호(int)을 전달받아 GUEST 테이블에 저장된 행을 검색하여 검색된 게시글(GuestDTO 객체)을 반환하는 메소드
	...
	//GUEST 테이블에 저장된 모든 행을 검색하여 검색된 게시글들(List 객체)을 반환하는 메소드
