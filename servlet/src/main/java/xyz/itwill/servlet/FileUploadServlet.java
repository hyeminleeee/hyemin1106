...
public class FileUploadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		//=> 이클립스에 생성된 업로드 디렉토리가 아닌 웹디렉토리에 전달받아 업로드되어 저장
		//WAS 프로그램이 실행(Start)될 때 이클립스 작업 디렉토리의 파일을 이용해 웹디렉토리의
		...
		//기능을 제공하기 위한 객체
		//=> MultipartRequest 객체를 생성하면 모든 전달 파일을 자동으로 서버 디렉토리에 저장 - 무조건적인 파일 업로드
		...
		//=> saveDirectory : 전달 파일을 저장할 서버 디렉토리의 파일 시스템 경로 전달 - 업로드 디렉토리
		...
		//MultipartRequest.getOriginalFileName(String name) : [multipart/form-data]로 전달된 입력파일의
		//이름을 얻어와 반환하는 메소드
		String fileone=mr.getOriginalFileName("fileone");
		String filetwo=mr.getOriginalFileName("filetwo");
		
		//MultipartRequest.getFilesystemName(String name) : 전달파일이 서버디렉토리에 업로드 처리된
		//업로드 처리된 파일의 이름을 얻어와 반환하는 메소드
		//이름을 얻어와 반환하는 메소드
