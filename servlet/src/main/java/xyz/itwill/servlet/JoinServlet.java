...
public class JoinServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//파일에 저장하여 전달
		//=> 리퀘스트 메세지 몸체부에 저장된 문서파일을 서블릿은 기본적으로 서유럽어로(ISO-8859-1)로
		...
		*/
		...
		//하나의 이름으로 다수의 값이 전달될 경우 getParameter() 메소드 대신 getParameterValues()
		//메소드를 호출하여 전달되는 모든 값을 배열로 반환받아 사용
		...
		//전달값에 
		out.println("<p>자기소개 = <br>"+pofile.replace("\n", "<br>")+"</p>");
		out.println("</body>");
		out.println("</html>");
