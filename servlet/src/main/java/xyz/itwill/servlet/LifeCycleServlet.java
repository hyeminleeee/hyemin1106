...
//=> 서블릿은 컨테이너에 의해 관리되는 객체 - 컴포넌트(Component) : JavaBean(객체)
@WebServlet({ "/LifeCycleServlet", "/life.itwill" })
public class LifeCycleServlet extends HttpServlet {
	...
	//클라이언트가 서블릿을 최초 요청할 때 WAS 프로그램에 의해 서블릿 클래스를 객체로 생성한 후
	//한번만 호출되는 메소드
	...
	//=> 전달값을 반환받아 데이터를 처리하고 응답 파일을 생성하는 명령 작성 - 요청 처리 메소드
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("</head>");
		out.println("<body>");
		out.println("<h1>서블릿의 생명주기(Servlet)</h1>");
		out.println("<hr>");
