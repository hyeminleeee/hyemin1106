...
//=> SQL 명령은 [EXIT] 명령을 입력하기 전까지 반복되어 실행되도록 작성 - 대소문자 미구분
...
public class SqlMinusApp {
	public static void main(String[] args) throws SQLException, IOException {
		...
		while(true) {
			...
			//키보드로 SQL 명령을 입력받아 저장 - 키보드로 입력받은 SQL 명령의 앞과 뒤에 존재하는
			//모든 공백을 제거한 후 변수에 저장
			...
			try {
				...
				if(stmt.execute(sql)) {//SELECT 명령을 전달하여 실행한 경우
					...
					if(rs.next()) {//검색행이 있는 경우
						...
						System.out.println("===============================================");
						//검색행의 컬럼명(별칭)을 반환받아 출력 처리하기 위한 반복문
				...
				} else {//SELECT 명령 외의 SQL 명령을 전달하여 실행한 경우
