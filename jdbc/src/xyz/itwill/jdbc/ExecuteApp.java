...
public class ExecuteApp {
	public static void main(String[] args) throws SQLException {
		...
		*/
		...
		//=> true 반환 : DQL(SELCET) 명령을 전달하여 실행한 경우의 반환값
		boolean result=stmt.execute(sql);
		
		if(result) {//DQL(SELECT) 명령을 전달하여 실행한 경우
			//Statement.getResultSet() : DBMS 서버에 전달되어 실행된 DQL(SELECT) 명령의 
			//실행결과(검색행)를 ResultSet 객체로 반환하는 메소드
			ResultSet rs=stmt.getResultSet();			
			while(rs.next()) {
				System.out.println("학번 = "+rs.getInt("no")+", 이름 = "
