...
//JDBC 프로그램은 기본적으로 자동 커밋 기능(Auto)이 활성화 되어 있는 DML 명령을 전달하여
//실행하면 무조건 자동으로 커밋 처리
//=> 프로그램 실행시 예외가 발생될 경우 예외 발생전 전달되어 실행된 DML 명령에 대한 롤백 처리 불가능

//JDBC 프로그램에서는 자동 커밋 기능을 비활성화 처리하고 프로그램 실행시 예외가 발생되지 않는
//경우 커밋 처리하고 예외가 발생된 경우 롤백 처리하는 것을 권장

//STUDENT 테이블에 저장된 행(학생정보)에서 학번이 [2000]인 학생의 이름을 [임꺽정]으로 변경하고
...
public class TransactionControlApp {
	public static void main(String[] args) {
		...
		try {
			...
			//Connection.setAutoCommit(boolean autoCommit) : 자동 커밋 기능을 활성화 상태(true - 기본값)
			//또는 비활성화 상태(false)로 변경하는 메소드
			...
			//if(con!=null) throw new Exception();//인위적으로 예외 발생
			...
			System.out.println("[에러]프로그램 실행에 예기치 못한 오류 발생");
