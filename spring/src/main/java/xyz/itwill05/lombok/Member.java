...
//1. Lombok 라이브러리를 프로젝트에 빌드 처리 - 메이븐 사용 : pom.xml 변경
...
//이클립스(STS3) 실행
// => eclipse.ini(STS.ini) 파일에서 [-javaagent:lombok.jar]변경 - lombok.jar 파일의 절대경로를 변경
...
//@Date : 클래스의 Setter 메소드, Getter 메소드, toString() 메소드, equals() 메소드, hashCode() 
//메소드를 제공하기 위한 어노테이션
// => VO 클래스(객체를 값처럼 비교할 수 있도록 작성된 클래스)를 선언할 때 사용
...
// => 원하는 필드만 초기화 처리할 수 있어 생성자보다 가독성이 좋으며 필드의 순서와 상관없이 초기화
