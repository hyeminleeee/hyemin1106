...
//@Component 어노테이션의 value 속성외 다른 속성이 없는 경우 속성값만 입력 가능
//@Component(value = "studentService")
//@Component("studentService")

//@Service : Service 클래스를 스프링 컨테이너가 관리할 수 있는 Spring Bean으로 등록하기 위한 어노테이션
// => 클래스의 이름을 자동으로 식별자(beanName)로 사용하지만 value 속성을 사용해 변경 가능
...
public class AnnotationStudentServiceImpl implements StudentService {
	...
	// => Setter Injection 기능을 사용하지만 필드에 Setter 메소드가 없어도 의존성 주입 가능
	...
	//@Resource : bean 엘리먼트의 autowire 속성값을 [byName]으로 설정한 것과 같은 방법으로 의존성 주입
	//@Inject : bean 엘리먼트의 autowire 속성값을 [byType]으로 설정한 것과 같은 방법으로 의존성 주입
	
	//문제점 : 필드의 자료형이 인터페이스인 경우 인터페이스를 상속받은 자식클래스가 Spring Bean으로
	//2개이상 등록된 경우 의존성 주입 실패 - 예외 발생
	//해결법-1 : 다수의 클래스 중 의존성 주입하고자 하는 클래스의 클래스의 식별자(beanName)를 필드명과 
	//동일하게 변경 - 동일한 자료형의 Spring Bean이 여러개인 경우 autowire 속성값을 [byName]으로 
	//설정한 경우와 같은 방법으로 의존성 주입
	//해결법-2 : 의존성 주입하고자 하는 클래스에 @Primary 어노테이션을 사용
	...
	//value 속성 : 클래스를 구분하기 위한 식별자(baenName)을 속성값으로 설정
