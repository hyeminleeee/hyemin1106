...
public class FileController {
	...
	*/
	...
	public String fileBoardWrite(@ModelAttribute FileBoard fileBoard
		...
		//FileBoardService 객체로 addFileBoard() 메소드를 호출하여 FILE_BOARD 테이블에 행으로 삽입 처리
	...
	//처리 클래스의 객체로 메소드를 호출해 메소드의 명령으로 응답 처리
	// => Spring Bean Configuration File(servlet-context.xml)에 BeanNameViewResolver 클래스를 
	//Spring Baen으로 등록 - ViewResolver 중 가장 우선순위가 높도록 설정
	@RequestMapping("/download")
	public String fileDownload(@RequestParam int num, Model model) {
		FileBoard fileBoard=fileBoardService.getFileBoard(num);
