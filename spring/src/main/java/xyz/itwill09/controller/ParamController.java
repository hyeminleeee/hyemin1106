...
public class ParamController {
	...
	//매개변수에 전달값을 제공받아 사용하기 위해서는 전달값에 대한 문자형태(캐릭터셋)을 변경 되도록
	//인코딩 필터 사용 - Front Controller 실행 전에 전달값에 대한 문자형태를 변경
	...
	*/
	...
	/*
	//전달값을 제공받아 저장하기 위한 매개변수에는 @RequestParam 어노테이션 사용 가능 - 권장
	...
	// => 전달값의 이름과 매개변수의 이름이 같은 경우 클라이언트로부터 전달된 값이 없는 경우
	...
	*/
	...
	// => value 속성외에 다른 속성이 없는 경우 속상값만 설정 가능
	// => 전달값의 이름과 매개변수의 이름이 다른 경우 전달값을 제공받아 매개변수에 저장하기 위해 사용
	public String action(@RequestParam(required = true, value = "food") String food, Model model) {
		model.addAttribute("food", food);
		return "param_display";
	}
	*/
	...
	//전달되는 값이 비어있는 경우 매개변수에 저장된 기본값 설정
	...
	public String form2() {
		return "paramValue_form";
	}
	
	
	
	//같은 이름으로 전달되는 값이 여러개인 경우 매개변수를 String 배열로 작성하여 배열의
	//전달값이 요소에 전달값이 차례대로 저장된 배열을 제공받아 매개변수에 저장하여 사용
	...
	public String action(@RequestParam("hobby") String[] hobbyList, Model model) {
		...
		return "paramValue_display";
	}
	
		
	//같은 이름으로 전달되는 값이 여러개인 경우 매개변수를 List 인터페이스로 작성해
	//전달값이 요소에 전달값이 차례대로 저장된 List 객체를 제공받아 매개변수에 저장하여 사용
	@RequestMapping(value = "/paramValues", method = RequestMethod.POST)
