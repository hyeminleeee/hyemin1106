...
//@Controller 어노테이션을 사용하여 Controller 인터페이스를 상속받지 않아도 요청 처리 클래스로 사용 가능
...
public class HelloController {
	...
	//요청 처리 메소드를 작성할 때 사용
	// => 클라이언트의 요청방식을 구분하여 요청 처리 메소드를 호출할 경우 @GetMapping, @PostMapping
	...
	public String helloViewName() {
		log.info("[/helloViewName] 페이지 요청 : HelloController 클래스의 hello() 메소드 호출");
		return "hello";
	}
	
	@RequestMapping("/helloMav")
	public ModelAndView helloModelAndView() {
		log.info("[/helloMav] 페이지 요청 : HelloController 클래스의 hello() 메소드 호출");
