...
public class JoinPointAdvice {
	...
	// => 타겟메소드에서 반환되는 값(객체)이 고정되어 있는 경우 Object 클래스 대신 반환값의
	//자료형을 사용해 매개변수 작성 가능
	...
	//After Returning Advice Method에서는 JoinPoint 인터페이스의 매개변수외에 Exception 클래스의
	//매개변수 작성 가능
	// => AspectJ 컴파일러는 Object 클래스의 매개변수에 타겟메소드에서 발생된 예외정보(Exception 객체)를
	//제공받아 저장
	// => 타겟메소드에서 발생되는 예외가 고정되어 있는 겨우 Exception 클래스 대신 예외를 저장
	...
	public Object display(ProceedingJoinPoint joinPoint) throws Throwable {//Aroud Advice Method
		...
		// => Throwable 객체(Error 클래스와 Exception 클래스의 부모클래스)가 발생되므로 반드시 
		//예외 처리 또는 예외 전달
