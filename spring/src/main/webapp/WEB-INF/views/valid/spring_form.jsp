	...
	<table>
		<tr>
			...
			<td>
				...
				<%-- => HTML input 태그의 type 속성값을 [text]로 설정한 것돠 동일한 태그 --%>
				<%-- path 속성 : 입력값을 전달하기 위한 이름을 속성값으로 설정 --%>
				<%-- => HTML input 태그의 name 속성과 id 속성을 설정하는 것과 동일 --%>
				<%-- => 입력값에 대한 유효성 검증 실패시 value 속성으로 Command 객체의 필드값을 제공받아 사용 --%>
				<form:input path="id"/>
		...
		<tr>
			...
			<td>
				<%-- Spring input 태그 : 문자열을 입력받아 전달하기 위한 태그 --%>
				<%-- => HTML input 태그의 type 속성값을 [password]로 설정한 것돠 동일한 태그 --%>
		...
		<tr>
			<td>성별</td>
			<td>
				<%-- Spring radiobutton 태그 : 나열된 목록 중 하나만 선택하여 값을 전달하기 위한 태그 --%>
				<%-- => HTML input 태그의 type 속성값을 [radio]로 설정한 것돠 동일한 태그 --%>
				<%-- 
				남자<form:radiobutton path="gender" cssClass="gender" value="남자"/>&nbsp;&nbsp;
				여자<form:radiobutton path="gender" cssClass="gender" value="여자"/>
				 --%>
				 
				<%-- Spring radiobuttons 태그 : 나열된 목록 중 하나만 선택하여 값을 전달하기 위한 태그 --%>
